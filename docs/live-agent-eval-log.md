...
### 빈 워크스페이스에 새 파일을 만드는 시나리오 — 지금까지 한 번도 측정한 적 없던 조건

이 로그의 기존 측정은 전부 **이미 있는 파일을 고치는** 시나리오(`src/demo_calc.py` 시드)
였다. 사용자가 빈 워크스페이스에 "계산기를 만들어줘"라고 요청하자 처음 보는 실패가 났다.

```
→ create_file(path='/workspace/src/calculator.py', ...)
✗ 작업 실패: Path traversal detected: '/workspace/src/calculator.py' is outside workspace
```

세션 워크스페이스는 `/workspace/.sessions/{id}`인데 모델이 `/workspace/src/**`를 그대로
썼다. 그 리터럴의 출처는 `prompts/system_prompt.txt`의 "Path restrictions" 절이었다 —
few-shot 예시가 아니라 **보안 규칙을 설명하는 문장 안의 경로**도 똑같이 복사된다.
고칠 파일이 이미 있는 시나리오에서는 모델이 실제 파일명을 앵커로 삼아 이 문제가 가려져
있었다.

측정(같은 이미지·모델·요청, 빈 워크스페이스, 프롬프트만 교체):

| 프롬프트 | 파일 생성 |
|---|---|
| 개정 전 원본 | 4/4 |
| `/workspace` 리터럴 2줄만 삭제 | 4/4 (1회는 첫 시도부터 정확) |
| 경로 규칙 전면 재작성(설명문 + 올바름/틀림 대조 추가) | **0/6** |

**전면 재작성이 0/6으로 무너진 이유는 아래 "backport 실패" 항목과 똑같다** — 금지 예시로
`/workspace/src/foo.py` 같은 리터럴을 새로 넣었더니, 모델이 그걸 피하는 대신 프롬프트
안의 다른 구체적 경로(Scenario 3의 `src/example_module.py`)로 새서 `read_file` →
"파일 없음" → 거짓 성공으로 끝났다. 같은 함정을 영어 기본 프롬프트에서 재현한 셈이다.
**채택한 것은 리터럴 2줄 삭제뿐이다**(오염원 제거, 새 리터럴 추가 없음).

### 경로 오류를 즉시 중단에서 재시도 가능으로 강등 — 효과가 가장 컸다

위 사고의 근본 문제는 프롬프트가 아니라 **경로 오타 한 번에 태스크 전체가 죽는 것**이었다.
`SecurityValidator`가 워크스페이스 이탈/허용 디렉토리 위반에 `PathPolicyError`(신설,
`SecurityError` 하위)를 던지고, 오케스트레이터가 이것만 복구 가능한 오류로 취급해
`"...retry with 'src/calculator.py'"` 힌트를 되먹이도록 바꿨다. `.env`/`.git`/위험 명령은
그대로 즉시 중단이다.

- 프롬프트를 안 고친 상태에서도 **4/4 복구** — 첫 액션에서 경로를 틀리고 다음 액션에서 교정.
- 최종 구성(리터럴 삭제 + 강등)으로 **11/12 파일 생성 성공**. 실패한 1회는 경로 오류 뒤
  `create_file` 재시도 대신 `run_tests`로 새서 아무것도 안 만들고 거짓 성공으로 끝났다.
- **모델은 프롬프트를 고쳐도 여전히 경로를 틀린다**(`/workspace/.sessions/{id}/...`,
  루트 `calculator.py` 등). 프롬프트로 막는 데는 한계가 있고, 되먹임이 실질적인 해법이다.

부수적으로 CLI가 `action_failed`/`security_violation` 이벤트를 아예 렌더링하지 않아
실패가 화면에서 사라지던 것과, `finish(success=false)`에도 초록색 "✓ 작업 완료"를 찍던
것을 고쳤다(`verification.suspicious` / `unbacked_change_claims`를 화면에 노출).
