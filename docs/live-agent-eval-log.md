...
**운영 결정**: 기본값은 한국어(`prompts/system_prompt.txt`)로 유지한다. 시연
시나리오는 양쪽 다 5/5라 교체해서 얻는 것이 없고, LAN 서버에서 실사용으로 검증된
것은 한국어 쪽이기 때문. 영어판은 `prompts/system_prompt.en.txt`로 함께 두고
`SYSTEM_PROMPT_PATH=/app/prompts/system_prompt.en.txt`로 언제든 전환할 수 있다.
